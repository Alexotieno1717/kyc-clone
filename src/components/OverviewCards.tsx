...
const OverviewCards = ({cardKey, title, amount}: OverviewCardsProps) =>{
    ...
    return (
        <div data-card-key={cardKey} className={`overflow-hidden rounded-2xl border shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${
            isCredits ? "border-emerald-100 bg-emerald-50/60" : "border-sky-100 bg-sky-50/60"
        }`}>
            <div className='space-y-4 px-5 py-5 md:px-6'>
                <div className='flex items-start justify-between'>
                    <h2 className="text-xs font-semibold tracking-[0.18em] text-gray-600" style={{ textTransform: 'uppercase' }}>{title}</h2>
